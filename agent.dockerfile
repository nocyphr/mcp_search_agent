...
COPY ./agent_requirements.txt /app/requirements.txt
RUN pip install -U --force-reinstall -r requirements.txt
COPY ./src /app/src
RUN pyinstaller --onefile src/server.py --name mcp_agent_server \
    --paths src --hidden-import=tiktoken_ext --hidden-import=tiktoken_ext.openai_public \
...
COPY --from=builder /app/dist/mcp_agent_server /usr/local/bin/mcp_agent_server
